...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"], NULL, 1);
	}
...

	{
		...
		float cos_value = glm::dot(forward, glm::vec3(0, 0, 1));

		modelMatrix = glm::rotate(modelMatrix, (float) acos(cos_value), glm::vec3(0, 1, 0));
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, int option)
{
	...
	int loc_elapsed_time = glGetUniformLocation(shader->program, "ElapsedTime");
	glUniform1f(loc_elapsed_time, (float) Engine::GetElapsedTime());

	int loc_option = glGetUniformLocation(shader->program, "Option");
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	GLenum wrapping_mode = GL_REPEAT;
