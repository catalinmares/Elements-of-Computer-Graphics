...
void main()
{
	...
	if (diffuse_light1 > 0) {
		specular_light1 = material_ks * pow(max(dot(normal, H2), 0), material_shininess);
	}
	...
	if (spot_light1 > cos(cut_off)) {
		out_color = vec4(object_color * vec3(light1), 1);
	} else if (spot_light2 > cos(cut_off)) {
		out_color = vec4(object_color *  vec3(light2), 1);
	} else {
		out_color = vec4(object_color * 0.1, 1);
	}
}