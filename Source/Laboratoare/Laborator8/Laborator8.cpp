...
void Laborator8::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.5f, 0.5f, 0.5f));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int angle = glGetUniformLocation(shader->program, "spotAngle");
	glUniform1f(angle, spotAngle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_UP)) {
			float c = cos(speed * deltaTime), s = sin(speed * deltaTime);
			glm::vec4 l = glm::vec4(glm::transpose(glm::mat4(1, 0, 0, 0,
				0, c, -s, 0,
				0, s, c, 0,
				0, 0, 0, 1)) * glm::vec4(lightDirection, 1));

			lightDirection = glm::vec3(l.x, l.y, l.z);
		}
		...
		if (window->KeyHold(GLFW_KEY_LEFT)) {
			...
			glm::vec4 l = glm::vec4(glm::transpose(glm::mat4(c, 0, s, 0,
				0, 1, 0, 0,
				-s, 0, c, 0,
				0, 0, 0, 1)) * glm::vec4(lightDirection, 1));
