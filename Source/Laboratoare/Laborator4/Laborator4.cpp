...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(0, 0, -5.f);
	angularStepOX2 += deltaTimeSeconds;

	modelMatrix *= Transform3D::Translate(0, 0, -2.f * step);
	modelMatrix *= Transform3D::RotateOX(-angularStepOX2);
	modelMatrix *= Transform3D::Translate(0, 0, 1.f);

	if (angularStepOX2 > M_PI) {
		angularStepOX2 = 0;
		step++;
	}
