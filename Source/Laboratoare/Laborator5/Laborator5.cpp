...
void Laborator5::Init()
{
	...
	fieldOfView = RADIANS(60.f);

	projectionMatrix = glm::perspective(fieldOfView, window->props.aspectRatio, 0.01f, 200.0f);
...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(17, 0, 17));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(3, 0.5f, 2));
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-8, 8, 0));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(5, 1, .25f));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_O)) {
			projectionMatrix = glm::ortho(left, right, bottom, top, 0.f, 30.f);
		}
